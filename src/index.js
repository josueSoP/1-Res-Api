...
async function updateTaskHandler(req, res) {
  try {
    ...
    if (idKey === "id") {
      ...
      res.write("Consulta invalida Query");
      res.end();
    }
  } catch (err) {
    response.writeHead(400, { "Content-type": "text/plain" });
    response.write("Datos invalidos", err.message);
    response.end();
  }
}
...
async function deleteTaskHandler(req, res) {
  ...
  if (idKey === "id") {
    ...
    res.writeHead(400, { "Content-type": "text/plain" });
    res.write("Error: consulta invalida");
    res.end();
  }
}
...
async function inicioServidor() {
  try {
    ...
    console.log(`Sistema iniciado en el puerto http://${process.env.HOST}:${process.env.PORT}`)
    })
  }catch (err){
    console.log(err)
